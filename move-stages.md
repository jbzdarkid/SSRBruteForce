...
0. **Classify the input** — Decide what the press *means* given Stephen's situation: standing **on** a sausage → log-roll; facing/touching a **ladder** → climb; a press that only changes facing → **turn**; otherwise → plain **walk**.
...
8. **Resolve outcomes** — Reconnect a thrown fork, register hot-ground steps, check win (all cooked + home) / loss (burn).

## Why this order

- **Classify before simulating (0→1).** The same key is ambiguous — "up" can mean climb, walk, turn, or roll the log underfoot. The terrain around Stephen dictates which, so that decision must come first. Dispatch priority (log-roll → ladder → turn → walk) runs from most context-hijacking to default.
...
- **Outcomes read the settled world (8).** Win, loss, and fork reconnection are judgments about the resulting state, so they come last.
...
This is what makes a move **atomic**. Because even the reactions play out on the plan, an illegal outcome — a sausage lost off the map, a burn — is simply a refusal, with nothing to undo. The move never changes the world and then rolls it back; it settles everything first and applies the result once. (Gravity still reads the *pre-move* board to see which pieces the move disturbed — but only as a reference, not a rollback buffer.)

This is a deliberate break from the older engine, which reasoned by mutating shared scratch and restoring it on failure — checking consequences only after already changing things, which is exactly what the staged model avoids. Keeping the plan local and separate makes each move self-contained.

The lone exception is the **ladder**: a climb or descent changes Stephen's height rung by rung against the live world rather than through the plan, so it alone doesn't yet share the all-at-once guarantee.
